...
Responde Única y exclusivamente con JSON plano sin markdown ni bloques de código:
{"name":"nombre descriptivo completo del plato","kcal":0,"protein":0,"carbs":0,"fat":0,"confidence":0,"ingredients":["ingrediente con tipo y cocción"],"portions":["ingrediente: Xg aprox."],"aiNote":"crítica profesional con números exactos y recomendación concreta"}`;
...
Estima las calorías (kcal), proteínas (g), carbohidratos (g) y grasas (g) con la máxima precisión posible para una persona con objetivo ${goalName}.
Responde ÚNICAMENTE con un JSON plano y sin formato markdown ni código (sin las comillas triples de bloque de código):
{"kcal":120,"protein":10,"carbs":15,"fat":2,"aiNote":"breve explicación"}`;
...
function LoginScreen({ onLogin }) {
  ...
  const [email, setEmail] = useState("ryan@email.com");
  const [password, setPassword] = useState("123456");
  const [error, setError] = useState("");

  function submit() {
    if (name.trim().length < 2) return setError("Introduce tu nombre.");
    ...
    <div className="page login-page">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="phone login-phone">
        ...
        <Card className="login-card">
          ...
          <p className="muted">Accede con tu nombre para personalizar la app.</p>
          ...
          <label><User size={18} /><input value={name} onChange={(e) => setName(e.target.value)} placeholder="Tu nombre" /></label>
          <label><Mail size={18} /><input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="correo@ejemplo.com" /></label>
          <label><Lock size={18} /><input value={password} type="password" onChange={(e) => setPassword(e.target.value)} placeholder="Contraseña" /></label>

          {error && <div className="error">{error}</div>}
          <Button onClick={submit} className="black-btn" style={{ marginTop: "14px", width: "100%" }}>{mode === "login" ? "Entrar" : "Crear cuenta"}</Button>
        </Card>
      </motion.div>
    </div>
  );
...
function App() {
  ...
  const [goal, setGoal] = useState(() => localStorage.getItem("fitai_goal") || "definir");
  ...
  const [foodHistory, setFoodHistory] = useState(() => {
    const saved = localStorage.getItem("fitai_foodHistory");
    return saved ? JSON.parse(saved) : seedMeals;
  });
  const [workoutsHistory, setWorkoutsHistory] = useState(() => {
    const saved = localStorage.getItem("fitai_workoutsHistory");
    return saved ? JSON.parse(saved) : seedWorkouts;
  });
  const [weightHistory, setWeightHistory] = useState(() => {
    const saved = localStorage.getItem("fitai_weightHistory");
    return saved ? JSON.parse(saved) : [
      { date: "2026-05-18", weight: 72 },
      { date: "2026-05-19", weight: 71.8 },
      { date: "2026-05-20", weight: 71.5 },
      { date: "2026-05-21", weight: 71.2 },
      { date: "2026-05-22", weight: 70.5 },
      { date: "2026-05-23", weight: 70.0 }
    ];
  });
  const [geminiApiKey, setGeminiApiKey] = useState(() => localStorage.getItem("fitai_geminiApiKey") || DEFAULT_API_KEY);
  ...
  const [healthConnected, setHealthConnected] = useState(() => {
    const saved = localStorage.getItem("fitai_healthConnected");
    return saved ? JSON.parse(saved) : false;
  });
  const [subscription, setSubscription] = useState(() => localStorage.getItem("fitai_subscription") || "free");
  ...
  useEffect(() => {
    localStorage.setItem("fitai_goal", goal);
  }, [goal]);
  ...
  useEffect(() => {
    localStorage.setItem("fitai_foodHistory", JSON.stringify(foodHistory));
  }, [foodHistory]);

  useEffect(() => {
    localStorage.setItem("fitai_workoutsHistory", JSON.stringify(workoutsHistory));
  }, [workoutsHistory]);

  useEffect(() => {
    localStorage.setItem("fitai_weightHistory", JSON.stringify(weightHistory));
  }, [weightHistory]);

  useEffect(() => {
    localStorage.setItem("fitai_geminiApiKey", geminiApiKey);
  }, [geminiApiKey]);

  useEffect(() => {
    localStorage.setItem("fitai_darkMode", JSON.stringify(darkMode));
  }, [darkMode]);

  useEffect(() => {
    localStorage.setItem("fitai_healthConnected", JSON.stringify(healthConnected));
  }, [healthConnected]);

  useEffect(() => {
    localStorage.setItem("fitai_subscription", subscription);
  }, [subscription]);
  ...
  if (stage === "login") return <LoginScreen onLogin={(u) => { setUser(u); setStage("goal"); }} />;
